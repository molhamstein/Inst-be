...
module.exports = function (Uploadfile) {
  Uploadfile.afterRemote("upload", function (context, result, next) {
    ...
    let userId;
    if (context.req.accessToken) userId = context.req.accessToken.userId;
    let files = [];
    ...
    var urlWebThumbRootSave =
      urlFileRoot + "/" + "web-thumbnail" + "/download/";
    ...
    for (let index = 0; index < arrayFiles.length; index++) {
      ...
      if (folderName == "video") {
        getVideoDurationInSeconds(src + "/video/" + file.name)
          .then((duration) => {
            ...
            ffmpeg.ffprobe(
              ...
              function (err, metadata) {
                ...
                if (err) {
                  ...
                  if (oldWidth != 0) var res = oldWidth / oldHeight;
                  else var res = 2;
                  //console.log("res");
                  //console.log(res);
                  ...
                  ffmpeg(src + "/" + folderName + "/" + file.name).screenshots({
                    count: 1,
        ...
        getAudioDurationInSeconds(src + "/audio/" + file.name).then(
          (duration) => {
