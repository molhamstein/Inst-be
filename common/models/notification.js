...
module.exports = function (Notification) {
  ...
  Notification.createNotifications = function (
    arrayOfObjects,
    key,
    typeUSer,
    typeId,
  ...
  Notification.createGelpNotifications = async function (
    arrayOfObjects,
    ownerId,
    typeId,
    cb
  ) {
    let typeObject = await Notification.app.models.notificationType.findById(
      typeId
    );
    for (let index = 0; index < arrayOfObjects.length; index++) {
      const element = arrayOfObjects[index];
      arrayOfObjects[index]["typeId"] = typeId;
      if (ownerId) arrayOfObjects[index]["ownerId"] = ownerId;
    }
    ...
    if (arrayOfObjects[0] && arrayOfObjects[0].ownerType == "YOUTUBER") {
      for (let index = 0; index < appNotificationData.length; index++) {
        ...
        let youtuberToken = await Notification.app.models.fcmToken.find({
          where: { youtuberId: element.ownerId },
        });
        ...
        firebase
          .messaging()
          .sendToDevice(arrayYoutuberToken, messageObject)
