...
module.exports = function (Course) {
  ...
  Course.createNewPhysicalCourse = async function (
    data,
    supplies = [],
    imagesId = [],
    req,
    ...
    try {
      await Course.app.models.user.checkRoleBranchAdmin(
        data.instituteId,
        data.branchId,
        req
      );
  ...
  Course.publishOnlineCourse = async function (id, req, callback) {
    try {
      ...
      await Course.app.dataSources.mainDB.transaction(async (models) => {
        ...
        let followerDate = await follower.find({
          where: { youtuberId: oldCourse.youtuberId },
        });
        ...
        Course.app.models.notification.createGelpNotifications(
          userData,
          null,
          5
        );
  ...
  Course.updateOnlineCourse = async function (data, units, req, mainCallback) {
    try {
      var youtuberId = data["youtuberId"]
        ? data["youtuberId"]
        : req.accessToken.userId;
      await Course.app.dataSources.mainDB.transaction(async (models) => {
        const { course } = models;
        const { unit } = models;
        const { onlineSession } = models;
        const { media } = models;
        ...
        if (data.id != null) {
          ...
          let mainConfig = await config.findOne({
            where: { type: "COURSE_PROMO_CODE" },
          });
          data["youtuberId"] = data["youtuberId"]
            ? data["youtuberId"]
            : youtuberId;
          data["isStarted"] = true;
          ...
          let mainSubcategory = await subCategory.findById(
            data["subcategoryId"]
          );
          ...
          for (
            let index = 0;
            index < mainSubcategory.code.length / 3;
            index++
          ) {
            subCategoryTreeCode.push(
              mainSubcategory["code"].slice(0, (index + 1) * 3)
            );
          }

          let subcategoryTree = await subCategory.find({
            where: { code: { inq: subCategoryTreeCode } },
          });
        ...
        for (var i = 0; i < units.length; i++) {
          ...
          if (element.id != null) {
            ...
            await mainUnit.updateAttributes({
              nameEn: element.nameEn,
              descriptionEn: element.descriptionEn,
              descriptionAr: element.descriptionEn,
              nameAr: element.nameEn,
              onlineSessionsCount: element.onlineSessions
                ? element.onlineSessions.length
                : 0,
            });
          } else {
            mainUnit = await unit.create({
          ...
          for (var j = 0; j < element.onlineSessions.length; j++) {
            ...
            if (videoElement.id != null) {
              ...
              await mainVideo.updateAttributes({
                nameEn: videoElement.nameEn,
                nameAr: videoElement.nameEn,
                descriptionEn: videoElement.descriptionEn,
                descriptionAr: videoElement.descriptionEn,
              ...
              mainVideo = await onlineSession.create({
                courseId: oldCourse.id,
        ...
        let levelId = await Course.app.service.getLevelId(
          Course.app,
          mainYouTuber,
          {
            totalPoint: tempTotalPoint,
            totalSessionCreaterTime:
              mainYouTuber.totalSessionCreaterTime + createrSessionTime,
          }
        );
        ...
        await oldCourse.updateAttribute(
          "duration",
          oldCourse.duration + newCourseDuration
        );
        ...
        if (
          !isNewCourse &&
          mainCourse.status == "active" &&
          newSessionIds.length != 0
        ) {
          let youtuberCourseData = await youtuberCourse.find({
  ...
  Course.getOneCourse = async function (id, req, callback) {
    try {
      ...
      if (mainCourse == null) throw Course.app.err.notFound.courseNotFound();
      await Course.app.models.user.checkRoleBranchAdmin(
        mainCourse.instituteId,
        mainCourse.branchId,
        req
      );
  ...
  Course.getOnlineCourses = async function (
    searchKey,
    code,
    minPrice,
    maxPrice,
    ...
    try {
      ...
      let coursesIds = await Course.app.query.getOnlineCourses(
        Course.app,
        searchKey,
        code,
        minPrice,
      ...
      let courses = await Course.find({
        where: { id: { inq: coursesIds } },
        oeder: "createdAt DESC",
      });
  ...
  Course.getOneOnlineCourse = async function (id, req, callback) {
    try {
      ...
      if (userId) {
        mainCourse["promoCode"] =
          await Course.app.models.youtuberCoursePromoCode.findOne({
            where: { courseId: id, ownerId: userId },
          });
        mainCourse["rate"] = await Course.app.models.rate.findOne({
          where: { courseId: id, youtuberId: userId },
        });
        let mainYoutuberCourse = await Course.app.models.youtuberCourse.findOne(
          { where: { courseId: id, youtuberId: userId } }
        );
        if (mainYoutuberCourse || userId == mainCourse.youtuberId) {
          ...
          let onlineSessionWatch =
            await Course.app.models.onlineSessionWatch.find({ courseId: id });
          for (
            let indexUnit = 0;
            indexUnit < mainCourse.units.length;
            indexUnit++
          ) {
            ...
            for (
              let index = 0;
              index < elementUnit.onlineSessions.length;
              index++
            ) {
              const element = elementUnit.onlineSessions[index];
              let isWatchOnlineSession = onlineSessionWatch.find(function (
                obj
              ) {
              ...
              if (isWatchOnlineSession) {
                if (isWatchOnlineSession.status == "finished") {
                  mainCourse["units"][indexUnit]["onlineSessions"][index][
                    "isWatchOnlineSession"
                  ] = true;
                  onlineSessionFinishCount++;
                  mainCourse["finishLessonNumber"]++;
                  if (
                    mainCourse["units"][indexUnit]["onlineSessions"][index + 1]
                  ) {
                    mainCourse["nextLesson"] =
                      mainCourse["units"][indexUnit]["onlineSessions"][
                        index + 1
                      ];
                  } else if (
                    mainCourse["units"][indexUnit + 1] &&
                    mainCourse["units"][indexUnit + 1]["onlineSessions"][0]
                  ) {
                    mainCourse["nextLesson"] =
                      mainCourse["units"][indexUnit + 1]["onlineSessions"][0];
                  }
            ...
            mainCourse["units"][indexUnit]["onlineSessionFinishCount"] =
              onlineSessionFinishCount;
            if (
              onlineSessionFinishCount ==
              mainCourse["units"][indexUnit]["onlineSessionsCount"]
            ) {
              mainCourse["units"][indexUnit]["isCompletedUnit"] = true;
            }
          }
          if (
            mainCourse["finishLessonNumber"] == mainCourse["sessionsNumber"]
          ) {
  ...
  Course.startCourse = async function (id, req, callback) {
    try {
      ...
      if (mainCourse.isStarted) throw Course.app.err.notFound.courseIsStarted();
      if (mainCourse.typeCost == "course") {
        ...
        await Promise.all(
          studentInCourse.map(async (element, index) => {
  ...
  Course.editPhysicalCourse = async function (
    id,
    data,
    imagesId,
    req,
  ...
  Course.addTeacherToCourse = async function (
    id,
    teacherId,
    typePaid,
    value,
    ...
    try {
      ...
      if (oldTeacher) throw Course.app.err.course.teacherAlreadyInCourse();
      ...
      var newTeacherCourse = await Course.app.models.teacherCourse.findById(
        teacherCourse.id
      );
  ...
  Course.addSessionToCourse = async function (id, sessions, req, callback) {
    try {
      ...
      await Course.app.dataSources.mainDB.transaction(async (models) => {
        const { session } = models;
        const { studentSession } = models;
        const { studentCourse } = models;
        var sessionData = [];
        sessions.forEach((element) => {
          sessionData.push({
            venueId: element.venueId,
  ...
  Course.getTeacherInCourse = async function (
    id,
    filter = {
      where: {},
    },
    ...
    try {
      ...
      if (filter["limit"] == null) filter["limit"] = 10;
      ...
      var teacherInCourse = await Course.app.query.threeLevel(
        Course.app,
        "teacherCourse",
        "teacher",
        "user",
  ...
  Course.getTeacherInCourseCount = async function (
    id,
    where = {},
    req,
    callback
  ...
  Course.getSessionInCourse = async function (
    id,
    filter = {
      where: {},
    },
  ...
  Course.getStudentInCourse = async function (
    id,
    filter = {
      where: {},
    },
    ...
    try {
      ...
      var studentInCourse = await Course.app.query.threeLevel(
        Course.app,
        "studentCourse",
        "student",
        "user",
  ...
  Course.getStudentInCourseCount = async function (
    id,
    where = {},
    req,
    callback
  ...
  Course.rateCourseOnline = async function (id, data, req, callback) {
    ...
    await Course.app.dataSources.mainDB.transaction(async (models) => {
      ...
      let mainRate = await rate.findOne({
        where: { courseId: id, youtuberId: userId },
      });
      if (mainRate) {
        throw Course.app.err.global.authorization();
      ...
      let updateData = {
        ...
        avgRate:
          (mainCourse.totalRate + data.value) / (mainCourse.rateCount + 1),
      };
  ...
  function cheackFullnOneCourses(courseId, callback) {
    Course.findById(courseId, function (err, data) {
      ...
      if (data.maxCountStudent < data.countStudent + 1) callback(null, data);
      else callback();
    });
  }
  ...
  Course.homePageCourse = async function (req, callback) {
    try {
      ...
      let featuredCategories = await Course.app.models.subCategory.find({
      ...
      let allCourse = await Course.find({
        where: {
          status: "active",
          subcategoryId: { inq: featuredCategoriesId },
        },
      ...
      for (let index = 0; index < allCourse.length; index++) {
        ...
        let indexCategory = featuredCategories.findIndex(
          (x) => x.id == element.subcategoryId
        );
  ...
  Course.checkIsInCourse = function (courses, userId) {
    return new Promise(function (resolve, reject) {
      ...
      if (userId == null) {
        ...
        Course.app.models.youtuberCourse.find(
          { where: { youtuberId: userId, coursesId: { inq: coursesId } } },
          function (err, allCourseMember) {
            if (err) {
              reject(err);
            }
